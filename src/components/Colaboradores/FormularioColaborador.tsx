 ...
 * Clon independiente del Formulario de Dirigentes: mismas secciones y UI,
 * pero guarda en la tabla/RPCs propias del módulo de Colaboradores.
 *
...
export const FormularioColaboradorComponent: React.FC<FormularioColaboradorProps> = ({
  colaboradorId,
  onSuccess,
  onCancel,
}) => {
  // Estados
  const [formData, setFormData] = useState<FormularioColaborador>(FORMULARIO_INICIAL);
  ...
  const cargarDatosColaborador = async (id: string) => {
    setLoadingData(true);
    try {
      const colaborador = await ColaboradorService.obtenerColaboradorPorId(id);
      if (colaborador) {
        setPersonaId(colaborador.persona.id || null);
        setFormData({
          nombres: colaborador.persona.nombres,
          apellidos: colaborador.persona.apellidos,
          fecha_nacimiento: colaborador.persona.fecha_nacimiento || '',
          sexo: mapearSexoDesdeBackend(colaborador.persona.sexo),
          tipo_documento: colaborador.persona.tipo_documento || 'DNI',
          numero_documento: colaborador.persona.numero_documento || '',
          correo: colaborador.persona.correo || '',
          correo_institucional: colaborador.persona.correo_institucional || '',
          celular: colaborador.persona.celular || '',
          telefono: colaborador.persona.telefono || '',
          departamento: colaborador.persona.departamento || '',
          provincia: colaborador.persona.provincia || '',
          distrito: colaborador.persona.distrito || '',
          direccion: colaborador.persona.direccion || '',
          codigo_postal: colaborador.persona.codigo_postal || '',
          // Datos de salud (ahora vienen de persona - DRY)
          religion: colaborador.persona.religion || '',
          grupo_sanguineo: colaborador.persona.grupo_sanguineo || '',
          factor_sanguineo: colaborador.persona.factor_sanguineo || '',
          seguro_medico: colaborador.persona.seguro_medico || '',
          tipo_discapacidad: colaborador.persona.tipo_discapacidad || '',
          carnet_conadis: colaborador.persona.carnet_conadis || '',
          descripcion_discapacidad: colaborador.persona.descripcion_discapacidad || '',
          codigo_credencial: colaborador.codigo_credencial || '',
          grupo_id: colaborador.grupo_id || '',
          unidad: colaborador.unidad || '',
          cargo: colaborador.cargo || '',
          centro_estudios: colaborador.centro_estudios || '',
          ciclo_anio_estudios: colaborador.ciclo_anio_estudios || '',
          centro_laboral: colaborador.centro_laboral || '',
          cargo_laboral: colaborador.cargo_laboral || '',
          profesion: colaborador.persona.profesion || '',
          nivel_formacion: colaborador.nivel_formacion || '',
          fecha_sfh1: colaborador.fecha_sfh1 || '',
          aprobo_sfh1: colaborador.aprobo_sfh1 || false,
          tipo_membresia: colaborador.tipo_membresia || 'REGISTRO_ANUAL_REGULAR',
          fecha_inicio_membresia: colaborador.fecha_inicio_membresia || '',
          acepta_politica_proteccion: colaborador.acepta_politica_proteccion || false,
          acepta_codigo_conducta: colaborador.acepta_codigo_conducta || false,
          autoriza_cuenta_institucional: colaborador.autoriza_cuenta_institucional || false,
          autoriza_uso_imagen: colaborador.autoriza_uso_imagen || false,
          declara_sin_antecedentes_policiales: colaborador.declara_sin_antecedentes_policiales || false,
          declara_sin_antecedentes_judiciales: colaborador.declara_sin_antecedentes_judiciales || false,
          declara_sin_antecedentes_penales: colaborador.declara_sin_antecedentes_penales || false,
          detalle_antecedentes: colaborador.detalle_antecedentes || '',
          contacto_emergencia_nombre: colaborador.contacto_emergencia?.nombre || '',
          contacto_emergencia_telefono: colaborador.contacto_emergencia?.telefono || '',
          contacto_emergencia_parentesco: colaborador.contacto_emergencia?.parentesco || '',
          observaciones: colaborador.observaciones || '',
        });
      }
    } catch (error) {
      showToast('error', 'Error al cargar datos del colaborador');
    } finally {
  ...
  const calcularCompletitud = () => {
    ...
    const camposLlenos = todosLosCampos.filter((campo) => {
      const valor = formData[campo as keyof FormularioColaborador];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let result;
      if (isEditing && colaboradorId) {
        result = await ColaboradorService.actualizarColaborador(colaboradorId, formData);
      } else {
        result = await ColaboradorService.registrarColaborador(formData);
      }

      if (result.success) {
        showToast('success', isEditing ? 'Colaborador actualizado exitosamente' : 'Colaborador registrado exitosamente');
  ...
  const handleChange = (
    field: keyof FormularioColaborador,
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-slate-500">Cargando datos del colaborador...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 mt-1">
              Registro Institucional de Colaboradores
            </p>
          </div>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <SelectField
              ...
              onChange={(e) => handleChange('cargo', e.target.value as CargoColaborador)}
              error={errors.cargo}
              required
...
export default FormularioColaboradorComponent;
