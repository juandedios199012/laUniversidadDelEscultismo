/**
 * Componente de Gestión de Documentos para Colaboradores
 * Permite subir, ver y gestionar documentos como DNI, certificados, etc.
 *
 * Clon independiente del componente de Dirigentes: usa el servicio y las
 * tablas/bucket de Storage propios del módulo de Colaboradores.
 */
...
export const GestionDocumentos: React.FC<GestionDocumentosProps> = ({
  ...
  const cargarDocumentos = async () => {
    setLoading(true);
    try {
      const docs = await ColaboradorService.obtenerDocumentos(colaborador.id);
      setDocumentos(docs);
    } catch (error) {
  ...
  const subirArchivo = async (file: File) => {
    ...
    setUploading(true);
    try {
      const result = await ColaboradorService.subirDocumento(
        colaborador.id,
        file,
        tipoSeleccionado
      );
  ...
  const eliminarDocumento = async (documentoId: string) => {
    ...
    try {
      const result = await ColaboradorService.eliminarDocumento(documentoId);
      if (result.success) {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-500 mt-1">
                {colaborador.persona.nombres} {colaborador.persona.apellidos}
              </p>
            </div>
        ...
        >
          ...
          {loading ? (
            ...
            <EmptyState
              ...
              description="Comienza subiendo los documentos requeridos del colaborador"
            />
          ) : (
            <div className="space-y-3">
