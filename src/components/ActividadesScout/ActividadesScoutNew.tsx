...
import type { ActividadScout, Scout } from '../../lib/supabase';
...
export default function ActividadesScoutNew() {
  ...
  const cargarDatos = async () => {
    try {
      setLoading(true);
      const [actividadesData, scoutsData] = await Promise.all([
        ScoutService.getAllActividades(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Mapear los datos del formulario a los nombres correctos del service
      const actividadData = {
        nombre: formData.titulo.trim(), // Cambio: titulo -> nombre
        descripcion: formData.descripcion.trim(),
        fecha_inicio: formData.fecha_inicio,
        fecha_fin: formData.fecha_fin,
        lugar: formData.ubicacion.trim(), // Cambio: ubicacion -> lugar
        costo: parseFloat(formData.costo_estimado) || 0, // Cambio: costo_estimado -> costo
  ...
  const handleEditActividad = (actividad: ActividadScout) => {
    ...
    setFormData({
      ...
      hora_inicio: actividad.hora_inicio || '09:00',
      fecha_fin: actividad.fecha_fin || actividad.fecha_inicio,
      hora_fin: actividad.hora_fin || '17:00',
      ubicacion: actividad.ubicacion || '',
      rama_objetivo: actividad.rama_objetivo,
      dirigente_responsable: actividad.dirigente_responsable || '',
      costo_estimado: actividad.costo_estimado?.toString() || '0',
      materiales_necesarios: actividad.materiales_necesarios || '',
      objetivos_educativos: actividad.objetivos_educativos || '',
  ...
  const handleUpdateActividad = async () => {
    ...
    try {
      ...
      const actividadData = {
        ...
        materiales_necesarios: formData.materiales_necesarios.trim(),
        objetivos_educativos: formData.objetivos_educativos.trim(),
        observaciones: formData.observaciones.trim(),
  ...
  const estadisticas = {
    ...
    costoTotal: actividades.reduce((sum, a) => sum + (a.costo_estimado || 0), 0)
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#4A90E2] via-[#5BA3F5] to-[#7BB3F0] p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 text-center border border-white/20">
            <div className="text-3xl font-bold text-yellow-300">{estadisticas.enCurso}</div>
            <div className="text-blue-100">En Curso</div>
          </div>
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 text-center border border-white/20">
            <div className="text-3xl font-bold text-green-300">{estadisticas.completadas}</div>
            <div className="text-blue-100">Completadas</div>
          </div>
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 text-center border border-white/20">
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 p-8 mb-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <FormField label="Costo Estimado (S/)">
                <Input
                  type="number"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <FormField label="Objetivos Educativos">
                <textarea
                  value={formData.objetivos_educativos}
                  onChange={(e) => handleInputChange('objetivos_educativos', e.target.value)}
                ...
                <div key={actividad.id} className="p-6 hover:bg-white/5 transition-colors">
                  <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 text-sm text-blue-100">
                            ...
                            {actividad.costo_estimado && actividad.costo_estimado > 0 && (
                              <>
                                <span>•</span>
                                <span className="text-green-300 font-medium">S/ {actividad.costo_estimado}</span>
                              </>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getEstadoColor(actividad.estado || 'planificada')}`}>
                        {actividad.estado?.toUpperCase() || 'PLANIFICADA'}
                      </span>
                  ...
                  {/* Detalles adicionales */}
                  {(actividad.materiales_necesarios || actividad.objetivos_educativos || actividad.observaciones) && (
                    <div className="mt-4 pt-4 border-t border-white/10">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        {actividad.materiales_necesarios && (
                          <div>
                            <h4 className="font-medium text-white mb-1">Materiales</h4>
                            <p className="text-blue-100">{actividad.materiales_necesarios}</p>
                          </div>
                        )}
                        
                        {actividad.objetivos_educativos && (
                          <div>
                            <h4 className="font-medium text-white mb-1">Objetivos</h4>
                            <p className="text-blue-100">{actividad.objetivos_educativos}</p>
                          </div>
                        )}
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="space-y-3">
                      <div>
                        <span className="text-blue-100 font-medium">Título:</span>
                        <p className="text-white">{selectedActividad.titulo}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <span className={`ml-2 px-3 py-1 rounded-full text-xs font-medium border ${getEstadoColor(selectedActividad.estado || 'planificada')}`}>
                          {selectedActividad.estado?.toUpperCase() || 'PLANIFICADA'}
                        </span>
                      </div>
                      <div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div>
                        <span className="text-blue-100 font-medium">Fecha:</span>
                      ...
                      <div>
                        <span className="text-blue-100 font-medium">Horario:</span>
                        <p className="text-white">
                          {selectedActividad.hora_inicio}
                          {selectedActividad.hora_fin && ` - ${selectedActividad.hora_fin}`}
                        </p>
                      </div>
                      {selectedActividad.ubicacion && (
                        <div>
                          <span className="text-blue-100 font-medium">Ubicación:</span>
                          <p className="text-white">{selectedActividad.ubicacion}</p>
                        </div>
                      )}
                      ...
                      {selectedActividad.costo_estimado && selectedActividad.costo_estimado > 0 && (
                        <div>
                          <span className="text-blue-100 font-medium">Costo:</span>
                          <p className="text-white">S/ {selectedActividad.costo_estimado}</p>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {(selectedActividad.materiales_necesarios || selectedActividad.objetivos_educativos || selectedActividad.observaciones) && (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {selectedActividad.materiales_necesarios && (
                      <div>
                        <span className="text-blue-100 font-medium">Materiales Necesarios:</span>
                        <p className="text-white mt-1">{selectedActividad.materiales_necesarios}</p>
                      </div>
                    )}
                    
                    {selectedActividad.objetivos_educativos && (
                      <div>
                        <span className="text-blue-100 font-medium">Objetivos Educativos:</span>
                        <p className="text-white mt-1">{selectedActividad.objetivos_educativos}</p>
                      </div>
                    )}
