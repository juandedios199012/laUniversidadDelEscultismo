...
function DraggableChip({ id, pictograma, deshabilitado }: { id: string; pictograma: string; deshabilitado: boolean }) {
  ...
  return (
    <button
      ...
      className={`min-w-[64px] min-h-[64px] w-16 h-16 sm:w-20 sm:h-20 rounded-3xl shadow-lg flex items-center justify-center text-4xl
      ...
      `}
    >
      <span aria-hidden="true">{pictograma}</span>
    </button>
  );
}
...
function EmparejarDragAndDrop({ config, puntosBase, onComplete }: {
  ...
  const [pares] = useState(() => (config?.pares || []).slice(0, MAX_PARES_VISIBLES));
...
function TarjetaPaso({ paso, posicion, fueraDeLugar }: { paso: PasoOrdenable; posicion: number; fueraDeLugar: boolean }) {
  ...
  return (
    ...
    >
      ...
      {paso.pictograma && <span className="text-3xl shrink-0" aria-hidden="true">{paso.pictograma}</span>}
